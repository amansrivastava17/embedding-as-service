...
def create_data(_):
    ...
    if FLAGS.task == 0:
        ...
        with tf.io.gfile.GFile(corpus_info_path, "w") as fp:
    ...
    with tf.io.gfile.GFile(record_info_path, "w") as fp:
...
def _local_perm(inputs, targets, is_masked, perm_size, seq_len):
    ...
    index = tf.transpose(a=tf.reshape(index, [-1, perm_size]))
    index = tf.random.shuffle(index)
    index = tf.reshape(tf.transpose(a=index), [-1])
...
def get_dataset(params, num_hosts, num_core_per_host, split, file_names,
    ...
    def parser(record):
        ...
        if num_predict is not None:
            ...
            indices = tf.boolean_mask(tensor=indices, mask=bool_target_mask)

            ##### extra padding due to CLS/SEP introduced after prepro
            actual_num_predict = tf.shape(input=indices)[0]
            ...
            ##### target
            target = tf.boolean_mask(tensor=target, mask=bool_target_mask)
